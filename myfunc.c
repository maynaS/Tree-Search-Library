...
void add_node(Node *parentptr, int n, Node new_node)
{   
   if(parentptr[new_node->self] == NULL)
   parentptr[new_node->self] = new_node;
   ...
   return;
       ...
       int j =0;
      while (parentptr[new_node->parent]->children[j] != NULL)
        {
            j++;
        }
        parentptr[new_node->parent]->children[j] = new_node;
          parentptr[new_node->parent]->numchild ++;
        new_node->depth =   parentptr[new_node->parent]->depth +1 ;
   }
}
